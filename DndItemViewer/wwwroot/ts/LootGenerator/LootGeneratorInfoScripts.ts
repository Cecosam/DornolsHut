...
function showRarities(): void {
    let input: HTMLInputElement = document.getElementById("CrInputInModal") as HTMLInputElement;
    if (isNumeric(input.value) == true) {
        let inputAsNumber: number = (input.value as unknown) as number;
        ...
        let commonItemDropRate: number = 10;
        let creatureRating: number = inputAsNumber;
        let bias: number = creatureRating / 4;
        bias = bias * bias;
